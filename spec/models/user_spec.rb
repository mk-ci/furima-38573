  ...
  describe '新規登録' do
    ...
    context 'ユーザー情報を新規登録できないとき' do
      ...
      it '名字は、全角（漢字・ひらがな・カタカナ）以外では登録できない' do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include('姓（全角）名前は全角（漢字・ひらがな・カタカナ）で入力してください')
      end
      it '名前は、全角（漢字・ひらがな・カタカナ）以外では登録できない' do
        @user.first_name = 'rikutarou'
        @user.valid?
        expect(@user.errors.full_messages).to include('名（全角）名前は全角（漢字・ひらがな・カタカナ）で入力してください')
      end
      ...
      it '名前カナ(全角)名字は、全角（カタカナ）以外では登録できない' do
        @user.last_kana = '山田'
        @user.valid?
        expect(@user.errors.full_messages).to include('姓（カナ）名前は全角カタカナで入力してください')
      end
      it '名前カナ(全角)名前は、全角（カタカナ）以外では登録できない' do
        @user.first_kana = 'りくたろう'
        @user.valid?
        expect(@user.errors.full_messages).to include("名（カナ）名前は全角カタカナで入力してください")
      end
